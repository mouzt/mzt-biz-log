package com.mzt.logserver.util;
...
/**
 * @author feichaoyu
 * created on 2021/11/17 4:44 下午
 */
public class RandomPOJOUtils {
    ...
    @SafeVarargs
    public static <T> List<T> randomPojoList(int expectSize, Class<T> clazz, Consumer<T>... consumers) {
        return Stream.iterate(0, i -> i).limit(expectSize).map(o -> randomPojo(clazz, consumers))
                .collect(Collectors.toList());
    }
}
